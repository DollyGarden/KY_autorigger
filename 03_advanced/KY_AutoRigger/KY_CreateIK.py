...
def ik_handles():
    ...
    spine_pos = []
    for i in enumerate(spines):
        spine_pos.append(cmds.xform(spines[i], q=True, t=True, ws=True))
    ...
    for j in enumerate(spine_pos):
        cmds.curve('SpineCurve', a=True, p=[(spine_pos[j][0], spine_pos[j][1], spine_pos[j][2])])
